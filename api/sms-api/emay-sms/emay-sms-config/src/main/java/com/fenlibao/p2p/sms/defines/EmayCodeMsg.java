...
public enum EmayCodeMsg {
    ...
    EmayCodeMsg(int errorcode, String source, String errmsg) {
        this.errorcode = errorcode;
        this.errmsg = errmsg;
        this.source = source;
    }

    public static EmayCodeMsg handCode(Object code) {
        if(code instanceof Integer) {
            int returnCode = Integer.parseInt(code.toString());
            for(EmayCodeMsg emayCodeMsg : values()) {
                if(returnCode == emayCodeMsg.getErrorcode()) {
                    return emayCodeMsg;
                }
            }
        }
