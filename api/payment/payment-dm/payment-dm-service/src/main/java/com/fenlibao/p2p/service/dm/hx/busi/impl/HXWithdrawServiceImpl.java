...
import com.fenlibao.p2p.service.dm.hx.busi.HXWithdrawService;
 ...
 */
@Service
public class HXWithdrawServiceImpl extends HXOrderProcessImpl implements HXWithdrawService {

    @Resource
    private WithdrawProcessDao withdrawProcessDao;

    @Override
    public String withdraw(int orderId, int userId, int clientType, String uri) throws Exception {
        HXTradeType tradeType = HXTradeType.TX;//华兴交易类型
        String tradeCode = HXTradeType.getTradeCode(tradeType, clientType);
        HXAccountInfo userInfo = hxUserService.getAccountInfo(userId);
        T6503 withdrawOrder = withdrawProcessDao.getOrder(orderId);//获取充值订单
        if (withdrawOrder == null) {
            logger.warn("提现订单不存在，userId=[{}],orderId=[{}]", userId, orderId);
        ...
        params.setAMOUNT(withdrawOrder.F03.toString());
        params.setREMARK("");
    ...
    protected void doConfirm(RespBusinessParams busiParams, HXOrder order) throws Exception {
        ...
        if (null != busiParams.getORDERSTATUS() && !HXConst.ORDERSTATUS_COMPLETED.equals(busiParams.getORDERSTATUS())) {
            logger.info("华兴提现非实时到账，userId=[{}],busiOrderId=[{}],state=[{}]", userId, busiOrderId, busiParams.getORDERSTATUS());
        ...
        if (t6501 != null && T6501_F03.DQR == t6501.F03) {
            ...
            T6503 withdrawOrder = withdrawProcessDao.getOrder(busiOrderId);
            if (withdrawOrder == null) {
                logger.warn("提现订单不存在，t6501.f01=[{}],userId=[{}]", busiOrderId, t6501.F08);
                throw new TradeException(TradeResponseCode.COMMON_RECORD_NOT_EXIST);
            }
            ...
            flow.F03 = TradeFeeCode.TX;
            flow.F04 = fundAccount.F01;
            flow.F07 = withdrawOrder.F03;//支出
    ...
    protected void doQuery(HXOrder order, ReqBusinessParams reqParams) throws Exception {
        reqParams.setTRANSDT(DateUtil.getYYYYMMDD(order.getCreateTime()));
    }
}
