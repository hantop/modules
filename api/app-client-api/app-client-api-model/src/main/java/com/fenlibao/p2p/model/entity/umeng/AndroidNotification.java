...
public abstract class AndroidNotification extends UmengNotification {
	...
	public boolean setExtraField(String key, String value,String type) throws Exception {
		JSONObject payloadJson = null;
		JSONObject extraJson = null;
		JSONObject modelDataJson = null;
		if (rootJson.containsKey("payload")) {
			payloadJson = rootJson.getJSONObject("payload");
		} else {
			payloadJson = new JSONObject();
		...
		if (extraJson.containsKey("modelData")) {
			modelDataJson = extraJson.getJSONObject("modelData");
		} else {
			modelDataJson = new JSONObject();
			extraJson.put("modelData", modelDataJson);
		}
		if(!StringUtils.isEmpty(type)&&type.equals("modelData")){
			modelDataJson.put(key, value);
		}else {
	...
	public void setPushType(String value) throws Exception {
		setExtraField("pushType", value,null);
	}

	public void setModelType(String value) throws Exception {
		setExtraField("modelType", value,null);
	}

	public void setActivityUrl(String value) throws Exception {
		setExtraField("activityUrl", value,"modelData");
	}
}
