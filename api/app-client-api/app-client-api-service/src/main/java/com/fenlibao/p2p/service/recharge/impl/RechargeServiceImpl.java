...
public class RechargeServiceImpl extends BaseAbstractService implements
	...
	private Map<String, String> getRequestParams(RechargeOrder order) {
		...
		// 是否为第三方支付测试
		String no_order = OrderUtil.genLlpRechargeOrderId(order.id,
				order.orderTime);
		boolean isTest = BooleanParser.parse(Payment.get(Payment.IS_PAY_TEST));
		if (isTest) {
			no_order = OrderUtil.genLlpRechargeOrderId_Test(order.id,
					order.orderTime);
		}
		// 订单号
		...
		params.put("dt_order",
				DateParser.format(order.orderTime, "yyyyMMddHHmmss"));
		// 连连支付支付单号
	...
	@Override
	public List<RechargeOrder> getDQROrder(Date requestTime) {
		return rechargeDao.getDQROrder(requestTime);
	}
						...
						if ("0000".equals(retParams.get("ret_code"))) {
							...
							if ("SUCCESS".equals(result_pay)) {
								...
								if (DateUtil.getDifferenceMin(orderTime
									...
									this.perfectPayInfo(retParams, order.userId);
								}
