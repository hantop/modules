...
public class FinacingServiceImpl implements FinacingService {
	...
	*/
	...
	@Override
	public List<Finacing> getFinacing(int userId, int bidId, int creditId) {
		String _userId = userId > 0 ? String.valueOf(userId) : null;
		String _bidId = bidId > 0 ? String.valueOf(bidId):null;
		String _creditId = creditId > 0 ? String.valueOf(creditId):null;

		return this.getFinacing(_userId, _bidId, _creditId);
	}
     ...
     */
	@Override
	public List<InvestInfo> getUserInvestList(int userId, int isUp, String timestamp) throws Exception{
		Date time = null;
		if (StringUtils.isNotEmpty(timestamp)) {
			time =  DateUtil.timestampToDateBySec(Long.valueOf(timestamp));
		}
		String[] bidStatus = new String[]{Status.TBZ.name(),Status.DFK.name(),Status.HKZ.name(), Status.YJQ.name()};
		return finacingDao.getUserInvestList(userId, isUp, time, bidStatus);
	}
	...
	public List<RepaymentInfoExt> getAllUserRepaymentItem(int userId, int type, Integer pageNo, Integer pagesize) {
		return finacingDao.getAllUserRepaymentItem(userId, type,pageNo,pagesize);
	}
	...
	public List<RepaymentInfoExt> getAllUserRepaymentItem(int userId, int type, VersionTypeEnum versionTypeEnum, Date timestamp, int mode) {
		return finacingDao.getAllUserRepaymentItem(userId, type, versionTypeEnum, timestamp, mode);
	}

	@Override
	public List<UserRepaymentAmout> userRepaymentAmout(Integer userId) {
		return finacingDao.userRepaymentAmout(userId);
	}

	@Override
	public List<UserRepaymentAmout> userRepaymentAmout(Integer userId, int versionTypeEnum) {
		return finacingDao.userRepaymentAmout(userId,versionTypeEnum);
	}

    @Override
    public List<PlanRepaymentDetail> getPlanRepaymentDetail(Integer planRecordId) {
		return finacingDao.getPlanRepaymentDetail(planRecordId);
    }
}
