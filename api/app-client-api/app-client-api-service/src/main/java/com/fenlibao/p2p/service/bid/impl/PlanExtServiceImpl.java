...
public class PlanExtServiceImpl implements PlanExtService {
    ...
    public int doBidForPlan(Connection connection, PlanBidVO planBidVO, int accountId) throws Throwable {
        ...
        try {
            Map<String, String> rtnMap = bidDmService.doBid(connection, planBidVO.getBidId(), planBidVO.getPurchaseAmount(), accountId, null, null, true, null);
            //余额投标订单
            ...
            //确认订单
            Throwable throwable2 = tenderOrderExecutor.confirmKernel(connection, IntegerParser.parse(rtnMap.get("orderId")), rtnMap, false, false, false, null);
            if (throwable2 != null) {
    ...
    @Override
    public void useRedPackets(int orderId, List<UserRedPacketInfo> userRedPacketInfos, int planId, int planRecordId, UserInfo userInfo, String fxhbIds, String investType, VersionTypeEnum versionTypeEnum) throws Exception {
        try {
            //更新标和红包的状态(计划)
...
//            }
    ...
    public void updateInvestPlanStatus(int planId) throws Exception {
        ...
        if(res > 0){
            int operatorId = terminationPlanService.getPmsUserId();
