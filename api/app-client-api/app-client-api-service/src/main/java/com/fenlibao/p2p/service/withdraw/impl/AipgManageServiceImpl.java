...
public class AipgManageServiceImpl extends AbstractAipgService implements
	...
	public String loan(Map<String, String> requsetParams) throws Throwable {
		...
		logger.debug("withdraw post url >>> [{}]", actionUrl);
		logger.info("开始向连连提交代付申请。。。");
		String retString = null;
		try {
			retString = HttpClientHandler.doPostJson(requsetParams, actionUrl);
		} catch (Throwable ex) {
			logger.error(String.format("向连连发起请求出现异常, no_order=[%s]", requsetParams.get("no_order")), ex);
			throw new BusinessException(ResponseCode.PAYMENT_WITHDRAW_REQUEST_FAIL_LLP);
		}
