...
public class RechargeManageServiceImpl extends BaseAbstractService implements IRechargeMangeService {
	...
	@Override
	public RechargeOrder addOrder(BigDecimal amount, int payCompanyCode, int userId, int isBind) throws Throwable {
		tradeService.validateAuth(userId);
		if (amount.compareTo(new BigDecimal(0)) <= 0 || payCompanyCode <= 0) {
			throw new BusinessException(ResponseCode.COMMON_PARAM_TYPE_WRONG); //金额或支付类型错误
		}
//		PaymentLimitVO paymentLimit = bankDao.getPaymentLimitByUserId(String.valueOf(userId));
//		if (paymentLimit == null) {
//			throw new BusinessException(ResponseCode.PAYMENT_RECHARGE_LIMIT_NOT_EXIST);
//		}
		String min = Payment.get(Payment.CHARGE_MIN_AMOUNT);
		String max = Payment.get(Payment.CHARGE_MAX_AMOUNT); //paymentLimit.getSingleLimit().multiply(new BigDecimal(10000));
		if (InterfaceConst.IS_BIND_BANK_CARD_YES == isBind) {
			min = Payment.get(Payment.MONEY_ORDER);
		}
 		if (amount.compareTo(new BigDecimal(min)) < 0) {
			throw new BusinessException(ResponseCode.PAYMENT_RECHARGE_LIMIT_LOW);
		}
		if (amount.compareTo(new BigDecimal(max)) > 0) {
			throw new BusinessException(ResponseCode.PAYMENT_RECHARGE_LIMIT_EXCEED);
		}
			...
			try (PreparedStatement pstmt = connection.prepareStatement(
				...
				try (ResultSet resultSet = pstmt.executeQuery()) {
					if (resultSet.next()) {
						...
						order.userId = resultSet.getInt(6);
					}
				}
			}
		}
