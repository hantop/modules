...
public class PlanBidHandler {
    ...
    public void releasePlan(PlanType planType, BigDecimal sumAmount, List<BidForPlanVO> tempBindBids) {
        ...
        // 是否有营销设置
        if(planType.getTargetUser() == 1 || planType.getTotalUserAssets() != null
                || planType.getUserInvestingAmount() != null  || planType.getUserAccumulatedIncome() != null) {
            planBidDao.addMarketSetting(planTypeTemp.getId(), planType.getTargetUser(), planType.getTotalUserAssets(), planType.getUserInvestingAmount(), planType.getUserAccumulatedIncome(), 1);
        }
        // 是否有标签设置
        if (StringUtils.isNotEmpty(planType.getLabel()) || StringUtils.isNotEmpty(planType.getComment())) {
            planBidDao.addProductLabel(planTypeTemp.getId(), planType.getLabel(), planType.getComment(), 1);
        }
        ...
        // 批量更新，计划绑定的标的还款费率
        Map<String, Object> bidRates = new HashMap<>();
        bidRates.put("rate", planType.getRate());
        bidRates.put("bindBids", tempBindBids);
        planBidDao.updateBidsRate(bidRates);
        // 发布计划
