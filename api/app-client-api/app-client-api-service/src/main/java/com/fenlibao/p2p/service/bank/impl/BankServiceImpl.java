...
public class BankServiceImpl implements BankService {
	...
	@Override
	public List<Map<String, Object>> buildBankCardsResult(List<BankCard> bankCards,int versiongControl) {
		List<Map<String, Object>> banks = new ArrayList<>();
		Map<String, Object> bankMap;
		for(BankCard card : bankCards) {
			bankMap = new HashMap<>();
			// 格式化身份证
			String bankNumEncrypt="";
			try {
				bankNumEncrypt = StringHelper.decode(card.getBankNumEncrypt());
			} catch (Throwable e) {
				e.printStackTrace();
			}
			String bankNumFormat = BankCardFormater.format(bankNumEncrypt);
			bankMap.put("bankName", card.getBankName());
			try {
				bankMap.put("bankPhone", card.getBankPhone() == null ? null : AES.getInstace().encrypt(card.getBankPhone()));
				if (versiongControl == 1) {//版本控制是否使用AES加密
					bankMap.put("bankNum", AES.getInstace().encrypt(bankNumEncrypt));
				} else {
	...
	@Override
	public List<Map<String, Object>> getBankCardsByUserId(int userId,int versiongControl) throws Throwable {
		List<BankCard> bankCards = bankDao.getBankCardsByUserId(userId);
		List<Map<String, Object>> resultList = buildBankCardsResult(bankCards,versiongControl);
		return resultList;
	}
...
//		String bankCodes = Payment.get(Payment.BANK_CODES); //支持的银行编码
		List<String> bankCodes = bankDao.getQYBankCode(); //TODO 放redis
		...
		if (retMap.size() > 0 && "0000".equals(retMap.get("ret_code"))) {
			if (!"2".equals(retMap.get("card_type"))) {
				throw new BusinessException(ResponseCode.PAYMENT_RECHARGE_ONLY_SUPPORT_DEBIT_CARD);
			}
			...
			throw new BusinessException(ResponseCode.PAYMENT_QUERY_CARD_INFO_FAIL);
		}
		return false;
	}
}
