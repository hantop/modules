...
import com.fenlibao.p2p.common.util.pagination.parser.impl.AbstractParser;
...
public class SqlUtil implements Constant {
	...
	private static final ThreadLocal<Page<Object>> LOCAL_PAGE = new ThreadLocal<Page<Object>>();
	...
	public static void setLocalPage(Page<Object> page) {
		LOCAL_PAGE.set(page);
	}
	 ...
	 * @return 返回Page对象
	 */
	public Page<Object> getPage(Object params) {
		Page<Object> page = getLocalPage();
		if (page == null) {
			if (params instanceof RowBounds) {
				RowBounds rowBounds = (RowBounds) params;
				if (offsetAsPageNum) {
					page = new Page<Object>(rowBounds.getOffset(), rowBounds.getLimit(),
							rowBoundsWithCount);
				} else {
					page = new Page<Object>(rowBounds, rowBoundsWithCount);
	 ...
	 * @return
	 */
	public static Page<Object> getPageFromObject(Object params) {
		int pageNum;
		int pageSize;
		...
		try {
			pageNum = Integer
					.parseInt(String.valueOf(getParamValue(paramsObject, "pageNum", true)));
			pageSize = Integer.parseInt(String
					.valueOf(getParamValue(paramsObject, "pageSize", true)));
		...
		Page<Object> page = new Page<Object>(pageNum, pageSize, count);
	...
	private Object _processPage(Invocation invocation) throws Throwable {
		...
		if (SqlUtil.getLocalPage() == null && rowBounds == RowBounds.DEFAULT) {
			...
			// 分页信息
			Page<Object> page = getPage(rowBounds);
			// pageSizeZero的判断
	...
	public static void testSql(Dialect dialect, String originalSql) {
		...
		if (dialect == Dialect.sqlserver) {
			setLocalPage(new Page<Object>(1, 10));
		}
