...
import com.fenlibao.p2p.common.util.pagination.sqlsource.PageDynamicSqlSource;
import com.fenlibao.p2p.common.util.pagination.sqlsource.PageProviderSqlSource;
...
public class MSUtils implements Constant {
	 ...
	 * @param args
	 */
	public void processPageMappedStatement(MappedStatement ms, SqlSource sqlSource,
			Page<Object> page, Object[] args) {
	...
	public MappedStatement getMappedStatement(MappedStatement ms, SqlSource sqlSource,
		...
		if (qs == null) {
			...
			qs = newMappedStatement(ms,
					getsqlSource(ms, sqlSource, parameterObject, suffix == SUFFIX_COUNT), suffix);
			if (parser.isSupportedMappedStatementCache()) {
				try {
	...
	public MappedStatement newMappedStatement(MappedStatement ms, SqlSource sqlSource, String suffix) {
		...
		if (suffix == SUFFIX_PAGE) {
			...
			ResultMap resultMap = new ResultMap.Builder(ms.getConfiguration(), id, int.class,
