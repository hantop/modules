...
import com.fenlibao.p2p.util.paginator.dialect.Dialect;
import com.fenlibao.p2p.util.paginator.domain.PageBounds;
import com.fenlibao.p2p.util.paginator.domain.PageList;
import com.fenlibao.p2p.util.paginator.domain.Paginator;
import com.fenlibao.p2p.util.paginator.support.PropertiesHelper;
import com.fenlibao.p2p.util.paginator.support.SQLHelp;
...
public class OffsetLimitInterceptor implements Interceptor{
    ...
    public void setPoolMaxSize(int poolMaxSize) {
        //多个线程池导致查询分页数据和查询总记录数的数据库的connection不一致，创建的新的connection无法释放，导致数据库连接池被占满。多线程存在问题，暂时使用单线程池处理
