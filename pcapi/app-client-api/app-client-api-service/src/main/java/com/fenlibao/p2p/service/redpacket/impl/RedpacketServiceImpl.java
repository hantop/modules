...
public class RedpacketServiceImpl implements RedpacketService {
    ...
    //@Transactional(readOnly = false)
    @Override
    public void grantRedPackets(Integer orderId, List<UserRedPacketInfo> userRedPacketInfos, String userId, String phoneNum, int tradeType, VersionTypeEnum versionTypeEnum) throws Exception {
        // 根据交易类型编码获取交易类型
        // 现金红包编码
  ...
  //junda.feng  2016-5-10 修改
    @Override
    public List getRedpackets(Map<String, Object> paramMap) {
        ...
        for (UserRedPacketInfo info : userRedPacketInfos) {
            ...
            redPacketMap.put("deadlineTips",
                    info.getInvestDeadline() != null ? String.format(InterfaceConst.INVESTMENT_DEADLINE_TIPS, info.getInvestDeadline()) : InterfaceConst.INVESTMENT_DEADLINE_TIPS_ALL);
    ...
    public void addUserRedpackets(List<UserRedPacketInfo> userRedPacketInfos, String userId, String phoneNum,
        ...
        if (StringUtils.isNotBlank(smsTemplate)) {
            ...
            if (StringUtils.isNotBlank(letterSuffix)) {
                privateMessageService.sendLetter(userId, InterfaceConst.PRIVATEMESSAGE_TITLE, content + letterSuffix);
            }
        }
    }
    ...
    @Override
    public UserCouponStatisticsInfo getRedpacketsStatistics(Map<String, Object> paramMap){
           return redpacketDao.getRedpacketsStatistics(paramMap);
    }
