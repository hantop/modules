...
public class WithdrawManageServiceImpl extends BaseAbstractService implements IWithdrawManageService {
        ...
        throws Throwable
    {
        ...
        {
            throw new BusinessException(ResponseCode.TRADE_WITHDRAW_LIMIT.getCode(), 
            		String.format(ResponseCode.TRADE_WITHDRAW_LIMIT.getMessage(), min, max));
        }
...
//          }
        tradeService.validateAuth(userId);
          if (isYuqi(userId))
          {
              throw new BusinessException(ResponseCode.TRADE_LOAN_OVERDUE.getCode(),
            		  ResponseCode.TRADE_LOAN_OVERDUE.getMessage());
          }
...
//                  }
                  throw new BusinessException(ResponseCode.TRADE_USER_NOT_RIGHT_PASSWORD.getCode(),
                		  ResponseCode.TRADE_USER_NOT_RIGHT_PASSWORD.getMessage()); //errorMsg
              }
              ...
              if (StringUtils.isBlank(bindStatus) || InterfaceConst.BANK_CARD_STATUS_WRZ.equals(bindStatus)) {
            	  throw new BusinessException(ResponseCode.TRADE_UNBOUND_BANK_CARD.getCode(),
            			  ResponseCode.TRADE_UNBOUND_BANK_CARD.getMessage());
              }
              ...
              if (txkcfs)
              {
                  ...
                  {
                      throw new BusinessException(ResponseCode.TRADE_AMOUNT_LESSTHAN_POUNDAGE.getCode(),
                    		  ResponseCode.TRADE_AMOUNT_LESSTHAN_POUNDAGE.getMessage());
                  }
              }
              else
              {
              ...
              {
                  throw new BusinessException(ResponseCode.TRADE_INSUFFICIENT_BALANCE.getCode(),
                		  ResponseCode.TRADE_INSUFFICIENT_BALANCE.getMessage());
              }
              ...
              if (amount.compareTo(wlzh.F06.subtract(offlineAmount)) > 0) {
            	  throw new BusinessException(ResponseCode.TRADE_OFFLINE_CANNOT_WITHDRAW.getCode(),
            			  String.format(ResponseCode.TRADE_OFFLINE_CANNOT_WITHDRAW.getMessage(), 
...
//                envionment);
        ...
        String content = "尊敬的用户"+t6110.F02+"： 您于"
                +DateTimeParser.format(new Date())+"提交的提现申请已成功，其中实际到账"
        		+funds.setScale(2, BigDecimal.ROUND_HALF_UP).toString()+"元，手续费"
                +poundage.setScale(2, BigDecimal.ROUND_HALF_UP).toString()+"元，请您注意查收。 感谢您对我们的关注与支持";
