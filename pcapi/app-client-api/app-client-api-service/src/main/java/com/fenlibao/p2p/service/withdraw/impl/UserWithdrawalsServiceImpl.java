...
public class UserWithdrawalsServiceImpl extends AbstractAipgService implements IUserWithdrawalsService {
		...
		try (Connection connection = getConnection();) {
			...
			try {
				...
				for (int id : ids) {
	                ...
	                {
	                    throw new BusinessException(ResponseCode.TRADE_BLACKLIST.getCode(),
	                    		ResponseCode.TRADE_BLACKLIST.getMessage());
	                }
	...
	private int addOrder(Connection connection, UserWithdrawals userWithdrawals, int userId)
		...
		try (PreparedStatement pstmt = connection
				.prepareStatement("INSERT INTO S65.T6503 SET F01 = ?, F02 = ?, F03 = ?, F04 = ?, F05 = ?, F06 = ?,F09 = ?")) {
