...
public class AipgBusManageServiceImpl extends BaseAbstractService implements
	...
	public List<Integer> getWithdrawingList() throws Exception {
		...
		try (Connection connection = getConnection()) {
			try (PreparedStatement pstmt = connection
					.prepareStatement("SELECT ord.F01 FROM S65.T6503 ord,S61.T6130 req WHERE ord.F09 = req.F01 and req.F09 = ? "
							+ " AND req.F08 <= DATE_SUB(now(), INTERVAL 1 HOUR) ORDER BY req.F08 ASC LIMIT 100 ")) { //获取一小时前的，避免正在提交就直接获取了
