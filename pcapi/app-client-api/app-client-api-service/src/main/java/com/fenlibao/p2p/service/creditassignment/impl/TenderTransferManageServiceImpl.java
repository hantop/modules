...
public class TenderTransferManageServiceImpl extends BaseAbstractService implements TenderTransferManageService {
	...
	@Override
    public int purchase(int transferId, int accountId) throws Throwable{
        ...
        try (Connection connection = getConnection()){
            try{
            	T6260 t6260 = isValidT6260(connection,transferId,accountId);
                if(t6260==null){
                    throw new BusinessException(ResponseCode.ZQZR_APPLY_DATA_NOT_FOUND.getCode(),ResponseCode.ZQZR_APPLY_DATA_NOT_FOUND.getMessage());
                }
            	// 债权转让费
    ...
    public T6260 isValidT6260(Connection connection, int transferId, int accountId) throws Exception{
        if (transferId <= 0){//线上债权转让申请不存在
            throw new BusinessException(ResponseCode.ZQZR_APPLY_DATA_NOT_FOUND.getCode(),ResponseCode.ZQZR_APPLY_DATA_NOT_FOUND.getMessage());
        }
        // 查询转让申请
...
//	                        }
    ...
    private List<ZqzrApplyRecord> getZrzList(Connection connection) throws Throwable {
        ...
        String sqlStr = "SELECT T6251.F01 AS zqId,T6251.F04 AS userId,T6260.F01 AS zqApplyId,T6260.F05 AS zqApplyTime FROM S62.T6260 INNER JOIN S62.T6251 ON T6260.F02 = T6251.F01 WHERE T6260.F07 = 'ZRZ' AND T6251.F08 = 'S' AND datediff(?, T6260.F05) > 5";
