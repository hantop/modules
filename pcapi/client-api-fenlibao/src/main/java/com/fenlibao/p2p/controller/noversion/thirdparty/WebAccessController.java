...
public class WebAccessController {
    ...
    HttpResponse autoLogin(@ModelAttribute BaseRequestForm  paramForm,
		...
		try {//AES解密
			...
			throw new BusinessException(ResponseCode.COMMON_DECRYPT_FAILURE.getCode(),ResponseCode.COMMON_DECRYPT_FAILURE.getMessage());
		}
    ...
    HttpResponse getAcessToken(@ModelAttribute BaseRequestForm paramForm,
		...
		try {//AES加密
			...
			throw new BusinessException(ResponseCode.COMMON_ENCRYPT_FAILURE.getCode(),ResponseCode.COMMON_ENCRYPT_FAILURE.getMessage());
		}
