    ...
    <div class="row">
        <div class="col-md-12">
            <ol class="breadcrumb">
                ...
                <li class="active"><b>加息券管理</b></li>
            </ol>
            <form:form class="form-inline" >
                <shiro:hasPermission name="rateCoupon:view">
                <div class="form-group">
                    <a class="btn btn-warning btn-sm"
                       href="${path}/reward/rateCoupon/rate-coupon-type-list"
                       role="button">加息券编辑</a>
                </div>
                </shiro:hasPermission>
                
                <shiro:hasPermission name="rateCoupon:template">
                <div class="form-group">
                    <a class="btn btn-primary btn-sm"
                       href="/static/template/加息券发放模板_短息来源内容.xlsx"
                       role="button">下载表格模板</a>
                </div>
                </shiro:hasPermission>
                
                <shiro:hasPermission name="rateCoupon:import">
                <div class="form-group">
            ...
            <hr>
            <form id="dataform" method="post" class="form-inline" action="${path}/reward/rateCoupon/">
			    <shiro:hasPermission name="rateCoupon:search">
			    <div class="form-group">
                    <label>激活日期：</label>
                    ...
                    <c:otherwise>
                        <c:forEach var="item" items="${list}">
                            <tr>
                                ...
                                <td>
                                    <shiro:hasPermission name="rateCouponDetail:view">
                                    &nbsp;&nbsp;
                                    <a style="text-decoration: underline;"
                                       href="${path}/reward/rateCoupon/rate-coupon-detial-list?grantId=${item.id}&granted=${item.granted==1?true:false}"
                                       role="button"
                                            >查看详情</a>
                                    </shiro:hasPermission>

                                    <shiro:hasPermission name="rateCoupon:grant">
                                    &nbsp;&nbsp;
									...
									<shiro:hasPermission name="rateCoupon:cancel">
									&nbsp;&nbsp;
							        <c:if test="${item.granted == 0}">
                                        <c:if test="${running == 0}">
...
<script type="application/javascript">
            ...
            $(file).on('change', function () {
                bootbox.confirm({
                    ...
                    callback: function (result) {
                        if (result) {
                            form.ajaxSubmit({
                                url: '${path}/reward/rateCoupon/import-rate-coupon',
	...
	function cancel(id){
		bootbox.confirm({
			...
			callback : function(result) {
				if(result){
					  <%--window.location.href="${path}/reward/rate-coupon/rate-coupon-cancel?id="+id;--%>
                    var data = {id:id};
                    $.ajax(
                            '${path}/reward/rateCoupon/rate-coupon-cancel',
                            {
    ...
    function send(id){
        bootbox.confirm({
            ...
            callback : function(result) {
                if(result){
                	/* var index = layer.load(0, {
                        shade: [0.4,'#fff',false] //0.1透明度的白色背景 rate-coupon-grant
                    }); */
                    <%--window.location.href="${path}/reward/rate-coupon/rate-coupon-grant?id="+id;--%>
                    var data = {id:id};
                    $.ajax(
                            '${path}/reward/rateCoupon/async-grant',
                            {
                                contentType: 'application/json; charset=utf-8',
                                data: JSON.stringify(data),
                                dataType: "json",
