    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form:form class="form-inline" >
               <%-- <shiro:hasPermission name="couponEdit:view"> --%>
                <div class="form-group">
                    <a class="btn btn-warning btn-sm"
                       href="${path}/reward/coupon/coupon-type-list"
                       role="button">加息券编辑</a>
                </div>
               <%-- </shiro:hasPermission> --%>
                
               <%-- <shiro:hasPermission name="coupon:template"> --%>
                <div class="form-group">
                    <a class="btn btn-primary btn-sm"
                       href="/static/template/加息劵发放模板_短息来源内容.xlsx"
                       role="button">下载表格模板</a>
                </div>
                <%-- </shiro:hasPermission> --%>
                <%-- <shiro:hasPermission name="coupon:import"> --%>
                <div class="form-group">
            ...
            <hr>
            <form id="dataform" method="post" class="form-inline" action="${path}/reward/coupon">
                    ...
                    <c:otherwise>
                        <c:forEach var="item" items="${list}">
                            <tr>
                                ...
                                <td>
                                   <%--  <shiro:hasPermission name="rateInterestDetail:view"> --%>
                                    &nbsp;&nbsp;
                                    <a style="text-decoration: underline;"
                                       href="${path}/reward/coupon/coupon-detial-list?grantId=${item.id}&granted=${item.granted==1?true:false}"
                                       role="button"
                                            >查看详情</a>
                                   <%--  </shiro:hasPermission>  --%>        
                                            
                                    <%-- <shiro:hasPermission name="rateInterest:grant">   --%>     
                                    &nbsp;&nbsp;
									...
									<shiro:hasPermission name="rateInterest:cancel">
									&nbsp;&nbsp;
							        <c:if test="${item.granted == 0}">
                                        <c:if test="${running == 0}">
...
<script type="application/javascript">
            ...
            $(file).on('change', function () {
                bootbox.confirm({
                    ...
                    callback: function (result) {
                        if (result) {
                            form.ajaxSubmit({
                                url: '${path}/reward/coupon/import-back-voucher',
	...
	function cancel(id){
		bootbox.confirm({
			...
			callback : function(result) {
				if(result){
					  <%--window.location.href="${path}/reward/coupon/back-voucher-cancel?id="+id;--%>
                    var data = {id:id};
                    $.ajax(
                            '${path}/reward/coupon/back-voucher-cancel',
                            {
        ...
        bootbox.confirm({
            ...
            callback : function(result) {
                if(result){
                    ...
                    <%--window.location.href="${path}/reward/coupon/back-voucher-grant?id="+id;--%>
                    var data = {id:id};
                    $.ajax(
                            '${path}/reward/coupon/async-grant',
                            {
                                contentType: 'application/json; charset=utf-8',
                                data: JSON.stringify(data),
                                dataType: "json",
