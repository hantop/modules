	...
	<div class="row">
		<div class="col-md-12">
			<ol class="breadcrumb">
				...
				<li class="active"><b>现金红包管理</b></li>
			</ol>
			<shiro:hasPermission name="cashRedPacket:template">
			    <button type="button" class="btn btn-primary btn-sm" onclick="window.location.href='/static/template/现金红包发放模板_短息来源内容.xlsx'">下载表格模板</button>
			</shiro:hasPermission>
			<shiro:hasPermission name="cashRedPacket:import">
			    <button type="button" class="btn btn-success btn-sm" id="importButton">导入表格</button>
			</shiro:hasPermission>
			<hr>
			<form id="dataform" method="post" class="form-inline" action="${path}/reward/red-packet/cash-red-packet-index">
			    <shiro:hasPermission name="cashRedPacket:search">
					...
					<tbody>
						<c:forEach items="${list}" var="v" varStatus="vs">
							<tr>
								...
								<td>${v.grantName}</td>
								<td>${v.operator}</td>
								<td  style="color:red">${v.sumPersent} </td>
                                <td>${v.countPersent}</td>
                                <td>${v.sysTypeLabel}</td>
								<td>
								    <shiro:hasPermission name="cashRedPacketDetail:view">
								    &nbsp;&nbsp; 
								    <c:choose>
                    					<c:when test="${v.inService != 1}">
								    <a style="text-decoration: underline;"	href="${path}/reward/red-packet/cash-red-packet-detail?grantId=${v.id}&granted=${v.granted==1?true:false}">查看详情</a>
								    </c:when>
                   						 <c:otherwise>
									...
									<shiro:hasPermission name="cashRedPacket:grant">
									&nbsp;&nbsp; 
									<c:if test="${v.granted==0}">
										<c:choose>
                    					<c:when test="${v.inService != 1}">
										<a style="text-decoration: underline;"
											href="#" onclick="javacript:send(${v.id}, ${v.sysType})">
											发送 </a>&nbsp;&nbsp;
                    					</c:when>
										<c:otherwise>
									...
									<shiro:hasPermission name="cashRedPacket:cancel">
									&nbsp;&nbsp;
									<c:if test="${v.granted==0}">
									<c:choose>
                    					<c:when test="${v.inService != 1}">
...
<script type="text/javascript">
            ...
            $(file).on('change',function() {
                ...
                bootbox.confirm({
                    ...
                    callback: function (result) {
                        if (result) {
                            form.ajaxSubmit({
                                url: '${path}/reward/red-packet/cash-red-packet-import',
	...
	function cancel(id){
		bootbox.confirm({
			...
			callback : function(result) {
				if(result){
					  <%--window.location.href="${path}/reward/red-packet/cash-red-packet-cancel?id="+id;--%>
					var data = {id:id};
					$.ajax(
							'${path}/reward/red-packet/cash-red-packet-cancel',
							{
	...
	function send(id, sysType){
		bootbox.confirm({
			...
			callback : function(result) {
				if(result){
					<%--window.location.href="${path}/reward/red-packet/cash-red-packet-grant?id="+id;--%>
					var data = {id: id, sysType: sysType};
					$.ajax(
							'${path}/reward/red-packet/cash-red-packet-grant',
							{
								contentType: 'application/json; charset=utf-8',
								data: JSON.stringify(data),
								dataType: "json",
