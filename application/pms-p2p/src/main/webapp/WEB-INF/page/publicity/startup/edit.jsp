    ...
    <div class="row">
        <div class="col-md-12">
            <ol class="breadcrumb">
                ...
                <li class="active"><a href="${path}/publicity/startup">启动图管理</a></li>
                <li class="active"><b>启动图${isNew ? "新增" : "编辑"}</b></li>
            </ol>
            <span class="badge" style="font-size: 18px;">${title}</span>
            <form:form id="dataform" role="form" class="form-horizontal" modelAttribute="startupImage">
                <form:hidden path="id" />
                ...
                <c:forEach items="${screenTypes}" var="screenType" varStatus="vs">
                    ...
                    <div class='form-group'>
                        ...
                        <c:choose>
                            <c:when test="${startupImage.imgFileMap != null && startupImage.imgFileMap.get(screenTypeId) != null}">
                                <a id="imgFile_${screenTypeId}" href="${startupImage.imgFileMap.get(screenTypeId).fullPath}"
                                   class="fancybox col-sm-2 control-label text-right" rel="gallery">
                                        ${startupImage.imgFileMap.get(screenTypeId).originalName}
                                </a>
                                <input id="hidden_publishName_${screenTypeId}" type="hidden" value="${startupImage.imgFileMap.get(screenTypeId).publishName}"/>
                                <input id="hidden_originalFilename_${screenTypeId}" type="hidden" value="${startupImage.imgFileMap.get(screenTypeId).originalName}"/>
                                <input name="hidden_relativePath_${screenTypeId}" id="hidden_relativePath_${screenTypeId}" type="hidden"
                                       value="${startupImage.imgFileMap.get(screenTypeId).relativePath}"/>
                                <input id="hidden_fileSize_${screenTypeId}" type="hidden" value="${startupImage.imgFileMap.get(screenTypeId).fileSize}"/>
                                <input id="hidden_fileType_${screenTypeId}" type="hidden" value="${startupImage.imgFileMap.get(screenTypeId).fileType}"/>
                ...
                <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                        ...
                        <a type="button" class="btn btn-default btn-sm" href="${path}/publicity/startup">返回</a>
                    </div>
                </div>
            </form:form>
        </div>
    ...
    <div class="modal-dialog plupload" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h5 class="modal-title text-center text-success" id="gridSystemModalLabel"><b>上传客户端启动图片</b></h5>
            </div>
            <div id="uploader">
...
<script type="text/javascript">
    ...
    function doFormSumit() {
        ...
        $.ajax({
            method: "post",
            url: path + "/publicity/startup/edit",
            dataType: "json",
            ...
            if (data.code == "200") {
                ...
                $("#tips").modal("show").on('hide.bs.modal', function (e) {
                    window.location.href = path + "/publicity/startup";
                });
            } else {
    ...
    function deleteImageQiniu(value) {
        ...
        $.ajax({
            method: "get",
            url: path + "/publicity/startup/delete/qiniu/",
            dataType: "json",
            data : {
                relativePaths : value
            }
    ...
    $("#uploader").plupload({
        ...
        url: $("#path").val() + '/publicity/startup/upload/qiniu',
        max_file_size: '3mb',
        max_file_count: 10,
        filters: [
