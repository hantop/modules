...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            ...
            <div>
                <form:form id="dataform" class="form-inline" method="post" action="${path}/publicity/advert"
                    commandName="advertImageForm">
                    <div class="form-group">
                        <label for="name">名称</label>
                        <form:input type="text" class="form-control input-sm" path="name" id="name" name="name" value="${advertImageForm.name}"/>
                    </div>
                    <div class="form-group">
                        <label for="advertType">广告类型</label>
                        <form:select path="advertType" cssClass="form-control">
                            <form:option value="">全部</form:option>
                            <form:options items="${advertimageEnums}" itemLabel="enumValue" itemValue="enumKey"></form:options>
                        </form:select>
                    </div>
                    <div class="form-group">
                        <label for="systemType">系统类型</label>
                        <form:select path="systemType" cssClass="form-control">
                            <form:option value="">全部</form:option>
                            <form:options items="${systemTypeEnums}" itemLabel="label" itemValue="value"></form:options>
                        </form:select>
                    </div>
                    <shiro:hasPermission name="publicityAdvert:search">
                        <button id="searchBtn" type="button" class="btn btn-primary btn-sm">查询</button>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="publicityAdvert:upload">
                        <a class="btn btn-success btn-sm" href="${path}/publicity/advert/new">新增</a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="publicityAdvert:delete">
                        ...
                        <tr>
                            ...
                            <td>
                                <shiro:hasPermission name="publicityAdvert:edit">
                                    <a href="${path}/publicity/advert/edit/${v.id}">编辑</a>
                                </shiro:hasPermission>
                                <shiro:hasPermission name="publicityAdvert:delete">
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    ...
                    $.ajax({
                        method: "put",
                        url: path + "/publicity/advert/status/",
                        dataType: "json",
                        data : {
                            id : id,
                            status : status
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    ...
                    $.ajax({
                        method: "delete",
                        url: path + "/publicity/advert/delete/" + id,
                        dataType: "json"
                    }).success(function (data) {
                        if (result) {
                            bootbox.alert({
