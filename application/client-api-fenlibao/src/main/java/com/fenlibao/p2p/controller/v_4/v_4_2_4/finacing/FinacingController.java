package com.fenlibao.p2p.controller.v_4.v_4_2_4.finacing;
 ...
 * 用户投资相关接口
 */
@RestController("v_4_2_4/FinacingController")
@RequestMapping(value = "finacing", headers = APIVersion.v_4_2_4)
...
//		investInfoDetailVO.setInterestPaymentType(infoDetail.getInterestPaymentType());
		...
		if ("0001".equals(infoDetail.getBidOrigin()) && infoDetail.getCgNum() == 2){  //来源是分利宝,存管标，散标才显示
			Map map = bidInfoService.getSignIdAndDocId(infoDetail.getBidId() + "");
			if (map != null && map.get("no_sensitive_sign_id") != null && map.get("no_sensitive_doc_id") != null){
				investInfoDetailVO.setAgreementUrl(ShangshangqianUtil.sdk.ViewContract(map.get("no_sensitive_sign_id").toString(), map.get("no_sensitive_doc_id").toString()));
			}else {
				String noSensitiveAgreementPath = bidInfoService.getNoSensitiveAgreementPath(infoDetail.getBidId() + "");
				investInfoDetailVO.setAgreementUrl(FileUtils.getFileURL(noSensitiveAgreementPath, Config.get("resources.server.path")));
			}
		}
