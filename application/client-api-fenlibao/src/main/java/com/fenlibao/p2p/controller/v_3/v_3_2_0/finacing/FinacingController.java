package com.fenlibao.p2p.controller.v_3.v_3_2_0.finacing;
 ...
 * 用户投资相关接口
 */
@RestController("v_3_2_0/FinacingController")
@RequestMapping(value = "finacing", headers = APIVersion.v_3_2_0)
public class FinacingController {
	...
	public HttpResponse getUserInvestRecordList(@ModelAttribute BaseRequestForm paramForm,
												String userId,String token,String timestamp,String isUp,String status ) throws Exception {
		...
		//用户的老计划和标
		List<InvestInfo> finacingList = tradeService.getUserProjectList(Integer.valueOf(userId), Integer.parseInt(isUp), timestamp, status);
		...
		List<PlanFinacingInfo> planList = planService.getPlanFinacing(Integer.valueOf(userId), Integer.parseInt(isUp), timestamp, status);
			...
			for (PlanFinacingInfo investInfo : planList) {
				...
				investInfoVO.setStatus(Integer.valueOf(status));//判断排序规则
					...
					if (investInfo.getNextRepaymentDate() == null) {
						...
						if(investInfo.getHoldStatus()==1){
							investInfoVO.setInvestStatus(3);
						}else if(investInfo.getHoldStatus()==2){
							investInfoVO.setInvestStatus(4);
						}
