 ...
 */
@RestController("v_4_2_4/BidController")
@RequestMapping(value = "bid", headers = APIVersion.v_4_2_4)
public class BidController {
    ...
    public HttpResponse doBid( @ModelAttribute BaseRequestFormExtend paramForm, String bidId,String sgSum,
        ...
        try {
            ...
            logger.debug("出借失败，bidId=[{}],出借金额=[{}],是否使用红包=[{}],userId=[{}],msg=[{}]", bidId, sgSum, fxhbIds, paramForm.getUserId(), apie.getMessage());
            apiResponse.setCodeMessage(apie);
        } catch (BusinessException busi) {
            logger.debug("出借失败，bidId=[{}],出借金额=[{}],是否使用红包=[{}],userId=[{}],msg=[{}]", bidId, sgSum, fxhbIds, paramForm.getUserId(), busi.getMessage());
            apiResponse.setCodeMessage(busi);
        } catch (Throwable throwable) {
            logger.error("出借失败，bidId=[{}],出借金额=[{}],是否使用红包=[{}],userId=[{}]", bidId, sgSum, fxhbIds, paramForm.getUserId());
            logger.error("出借失败", throwable);
