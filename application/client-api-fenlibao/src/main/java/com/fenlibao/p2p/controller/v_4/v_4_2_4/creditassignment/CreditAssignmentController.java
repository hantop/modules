 ...
 * @Package: com.fenlibao.p2p.controller.v_4.v_4_2_0.creditassignment
 * @author: laubrence
 * @date: 2015-10-22 上午9:34:57
 * @version: V1.0.0
 */
package com.fenlibao.p2p.controller.v_4.v_4_2_4.creditassignment;
...
import com.fenlibao.p2p.model.entity.bid.BidBorrowerInfoAfterLoan;
...
@RestController("v_4_2_4/CreditAssignmentController")
@RequestMapping(value = "creditassignment", headers = APIVersion.v_4_2_4)
public class CreditAssignmentController {
    ...
    HttpResponse transferInPurchase(HttpServletRequest request, @ModelAttribute BaseRequestForm paramForm,
                                    ...
                                    @RequestParam(required = true, value = "versionType") String versionType) throws Throwable {
        HttpResponse response = new HttpResponse();
        try {
            if (!paramForm.validate() || StringUtils.isEmpty(token) || StringUtils.isEmpty(userId) || StringUtils.isEmpty(applyforId) || (!versionType.equals("CG")&&StringUtils.isEmpty(dealPassword))||StringUtils.isEmpty(versionType)) {
                response.setCodeMessage(ResponseCode.EMPTY_PARAM.getCode(), ResponseCode.EMPTY_PARAM.getMessage());
                return response;
            }

            Integer canDebtTransfer = Integer.parseInt(Config.get("fenlibao.canDebtTransfer"));
            if (canDebtTransfer == 0){ //债转关闭
                response.setCodeMessage(ResponseCode.ZQZR_CLOSE);
                return response;
            }
            ...
            if(cgNum ==  VersionTypeEnum.PT.getIndex() ){
            ...
            if(cgNum==VersionTypeEnum.CG.getIndex()&&bidInfoService.isCGBid(Integer.valueOf(transferOutInfo.getBidId()))!=2){
                response.setCodeMessage(ResponseCode.BID_NOT_CG);
                return response;
            }
    ...
    HttpResponse transferInList(HttpServletRequest request, @ModelAttribute BaseRequestForm paramForm,
                                ...
                                @RequestParam(required = false, value = "versionType") String versionType) throws Exception {
        HttpResponse response = new HttpResponse();
        ...
        VersionTypeEnum vte = VersionTypeEnum.parse(versionType) == null ? VersionTypeEnum.PT : VersionTypeEnum.parse(versionType);
        List<TransferInInfo> transferInInfos = transferInService.getTransferInAndPlanInfoList(Integer.valueOf(userId), timestamp, transferStatus, vte);
        for (TransferInInfo transferInInfo : transferInInfos) {
            ...
            // 月标的计算公式：债权本金 + 原债权预期收益 - 债权价格) / 债权价格* 12 / 剩余持有月数
            // 换算债权预期收益率，暂时全部按天标的计算方式。

            Integer timeDiff = DateUtil.getDayBetweenDates(transferInInfo.getPurchaseTime(), transferInInfo.getExpireDate());
            double newYearyield = CalculateEarningsUtil.transferYieldForDay(transferInInfo.getOriginalAmount().add(transferInInfo.getExpectedProfit()).doubleValue(),
                    transferInInfo.getInvestAmount().doubleValue(), timeDiff);
            // transferInInfoVO.setYearYield(transferInInfo.getYearYield());
            transferInInfoVO.setYearYield(newYearyield);
    ...
    HttpResponse transferInDetail(HttpServletRequest request, @ModelAttribute BaseRequestForm paramForm,
        ...
        double newYearyield = CalculateEarningsUtil.transferYieldForDay(transferInInfo.getOriginalAmount().add(transferInInfo.getExpectedProfit()).doubleValue(),
                transferInInfo.getInvestAmount().doubleValue(), timeDiff);
        transferInInfoDetailVO.setYearYield(newYearyield);
...
//      transferInInfoDetailVO.setInterestPaymentType(transferInInfo.getInterestPaymentType());
        ...
        transferInInfoDetailVO.setCollectInterest(transferInInfo.getCollectInterest()>0?transferInInfo.getCollectInterest():0);
        transferInInfoDetailVO.setOriginalAmount(transferInInfo.getOriginalAmount().doubleValue());

        //获取借款人信息
        BidBorrowerInfo bbInfo = bidInfoService.getBidBorrowerInfo(Integer.valueOf(transferInInfo.getBidId())); //获取新的数据
        BidBorrowerInfoAfterLoan bidBorrowerInfoAfterLoan = bidInfoService.getBidBorrowerInfoAfterLoan(Integer.valueOf(transferInInfo.getBidId()));

        if(bbInfo!=null) {
            transferInInfoDetailVO.setRisk(bbInfo.getRisk());
            transferInInfoDetailVO.setBase(bbInfo.getBorrower_info());
            transferInInfoDetailVO.setBankTransaction(bbInfo.getBank_transaction());
            transferInInfoDetailVO.setCredit(bbInfo.getCredit());
        }
        ...
        //获取标信息
        ShopTreasureInfo info = this.bidInfoService.getBidInfo(transferInInfo.getBidId(), null);

        List<Map<String, Object>> projectInfo = bidInfoService.pieceTogetherProjectInfo(info);
        List<Map<String, Object>> borrowerInfo = bidInfoService.pieceTogetherBorrowerInfo(bbInfo, bidBorrowerInfoAfterLoan, info);
        transferInInfoDetailVO.setProjectInfo(projectInfo);
        transferInInfoDetailVO.setBorrowerInfo(borrowerInfo);
    ...
    HttpResponse transferOutCancel(HttpServletRequest request, @ModelAttribute BaseRequestForm paramForm,
                                   String token,
                                   String userId,
                                   String zqId,
                                   String versionType) throws Throwable {
        HttpResponse response = new HttpResponse();
        ...
        try {
            ...
            //添加前缀
            String code =InterfaceConst.CREDIT_NAME_PREFIX;

            code += tenderTransferManageService.cancel(Integer.valueOf(zqId), Integer.valueOf(userId));
    ...
    HttpResponse applayforList(@ModelAttribute BaseRequestForm paramForm,
                               ...
                               @RequestParam(required = false, value = "timestamp") String timestamp,String versionType) throws Exception{
        HttpResponse response = new HttpResponse();
        ...
        int totalpage = transferInService.getTransferOutListTotalPages(cgNum);
        List<TransferOutInfo> transferOutInfos = transferInService.getTransferOutList(curPage,null,cgNum);
    ...
    HttpResponse applayforDetail(@ModelAttribute BaseRequestForm paramForm,
                                 @RequestParam(required = false, value = "applyforId") String applyforId, VersionTypeEnum versionType) throws Exception{
        HttpResponse response = new HttpResponse();
        ...
        transferOutDetailVO.setLoanMonths(transferOutInfo.getLoanMonths());
        transferOutDetailVO.setLoanDays(transferOutInfo.getLoanDays());
        transferOutDetailVO.setBidYield(transferOutInfo.getYearYield());

        //获取借款人信息
        BidBorrowerInfo bbInfo = bidInfoService.getBidBorrowerInfo(Integer.valueOf(transferOutInfo.getBidId())); //获取新的数据
        BidBorrowerInfoAfterLoan bidBorrowerInfoAfterLoan = bidInfoService.getBidBorrowerInfoAfterLoan(Integer.valueOf(transferOutInfo.getBidId()));

        if (bbInfo == null) {
            BorrowerInfo bInfo = bidInfoService.getBorrowerInfo(Integer.valueOf(transferOutInfo.getBidId()));
            if (bInfo == null) {
                return response;
            }
            transferOutDetailVO.setInfoMsg(bInfo.getInfoMsg());
            transferOutDetailVO.setIdentify(bInfo.getIdentify());
            StringBuffer phoneBuffer = new StringBuffer(bInfo.getPhone());
            transferOutDetailVO.setPhone(phoneBuffer.replace(3, 7, "****").toString());
            if (StringUtils.isNoneBlank(bInfo.getCompany()) && bInfo.getCompany().length() >= 4) {
                ...
                transferOutDetailVO.setCompany(companyBuffer.replace(2, bInfo.getCompany().length() - 2, "*******").toString());
            } else {
                transferOutDetailVO.setCompany("");
            }
            transferOutDetailVO.setIncome(bInfo.getIncome());
            transferOutDetailVO.setIsCarCertified("TG".equals(bInfo.getIsCarCertified()) ? 1 : 0);
            transferOutDetailVO.setIsHouseCertified("TG".equals(bInfo.getIsHouseCertified()) ? 1 : 0);
        } else {
            transferOutDetailVO.setRisk(bbInfo.getRisk());
            transferOutDetailVO.setBase(bbInfo.getBorrower_info());
            transferOutDetailVO.setBankTransaction(bbInfo.getBank_transaction());
            transferOutDetailVO.setCredit(bbInfo.getCredit());
        }
        ...
        //获取标信息
        ShopTreasureInfo info = this.bidInfoService.getBidInfo(transferOutInfo.getBidId(), null);

        List<Map<String, Object>> projectInfo = bidInfoService.pieceTogetherProjectInfo(info);
        List<Map<String, Object>> borrowerInfo = bidInfoService.pieceTogetherBorrowerInfo(bbInfo, bidBorrowerInfoAfterLoan, info);
        transferOutDetailVO.setProjectInfo(projectInfo);
        transferOutDetailVO.setBorrowerInfo(borrowerInfo);
    ...
    HttpResponse applyforSubmit(@ModelAttribute BaseRequestForm paramForm,
                                ...
                                final String passedEarning,
                                String dealPassword,
                                final String versionType) throws Exception {
        HttpResponse response = new HttpResponse();
        try {
            if (!paramForm.validate() || StringUtils.isEmpty(zqId) || StringUtils.isEmpty(transferValue)) {
                response.setCodeMessage(ResponseCode.EMPTY_PARAM.getCode(), ResponseCode.EMPTY_PARAM.getMessage());
                return response;
            }
            ...
            if(versionType!=null&&versionType .equals( VersionTypeEnum.PT.getCode()) ){
                response.setCodeMessage(ResponseCode.NOT_SUPPURT_OUT_TRANFER);
                return response;
            }

            VersionTypeEnum versionTypeEnum = VersionTypeEnum.parse(versionType) == null ? VersionTypeEnum.PT : VersionTypeEnum.parse(versionType);
            if(VersionTypeEnum.PT.equals(versionTypeEnum)){
                if (StringUtils.isEmpty(dealPassword)) {
                    response.setCodeMessage(ResponseCode.EMPTY_PARAM.getCode(), ResponseCode.EMPTY_PARAM.getMessage());
                    return response;
                }
                //验证用户交易密码
