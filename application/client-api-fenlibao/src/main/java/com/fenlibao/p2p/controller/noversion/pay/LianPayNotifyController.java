		...
		try {
			...
			if (DigestUtil.checkReturnSign(params)) {
				...
				if ("SUCCESS".equals(Succeed) && TransID.indexOf("_") != -1) {
					...
					logger.info(String.format("no_order:[%s] charge fail,原因：[%s] ", TransID, params.get("info_order")));
				}
			} else {
            ...
            if (retParams.size() > 3) {
                ...
                if (DigestUtil.checkReturnSign(retParams)) {
                    ...
                    if ("SUCCESS".equals(result_pay)) {
                    	...
                    	String info_order = retParams.get("info_order");
                    	logger.info(String.format("no_order:[%s]交易失败（最终结果），原因：%s", no_order, info_order));
                    	if (info_order.length() > 100) {
                    		info_order = info_order.substring(0, 99);
                    	}
                        aipgBusManageService.withdrawFail(OrderUtil.getOrderIdByNoOrder(no_order), info_order);
                        this.stopNotify(response, writer);
                    }
                } else {
                ...
                logger.info(String.format("原因：%s", retParams.get("info_order")));
            }
		} catch (Throwable e) {
			logger.error(e.toString(), e);
		} finally {
