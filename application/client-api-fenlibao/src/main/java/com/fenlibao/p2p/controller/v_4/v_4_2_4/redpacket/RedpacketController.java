package com.fenlibao.p2p.controller.v_4.v_4_2_4.redpacket;
...
@RestController("v_4_2_4/RedpacketController")
@RequestMapping(value = "redpacket", headers = APIVersion.v_4_2_4)
public class RedpacketController {
    ...
    public HttpResponse share(BaseRequestFormExtend params, String bidId, int itemType,String newPlan) {
        ...
        try {
            ...
            logger.warn("出借分享红包失败,userId=[{}],bidId=[{}],msg=[{}]", params.getUserId(), bidId, busi.getMessage());
        } catch (Exception e) {
            response.setCodeMessage(ResponseCode.FAILURE);
            logger.error("出借分享红包失败,userId=[{}],bidId=[{}]", params.getUserId(), bidId);
            logger.error("出借分享红包失败", e);
        }
        return response;
    }
    ...
    public HttpResponse receive(BaseRequestForm params, String phoneNum, String redEnvelopeCode) {
        ...
        try {
            ...
            response.setCodeMessage(busi);
            logger.warn("领取出借分享红包失败,phoneNum=[{}],redEnvelopeCode=[{}],msg=[{}]", phoneNum, redEnvelopeCode, busi.getMessage());
        } catch (Exception e) {
            response.setCodeMessage(ResponseCode.FAILURE);
            logger.error("领取出借分享红包失败,phoneNum=[{}],redEnvelopeCode=[{}]", phoneNum, redEnvelopeCode);
            logger.error("领取出借分享红包失败", e);
        }
        return response;
    }
}
