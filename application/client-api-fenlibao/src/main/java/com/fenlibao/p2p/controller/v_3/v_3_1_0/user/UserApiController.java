package com.fenlibao.p2p.controller.v_3.v_3_1_0.user;
...
@RestController("v_3_1_0/UserApiController")
@RequestMapping(value = "user", headers = APIVersion.v_3_1_0)
public class UserApiController {
	...
	HttpResponse userAssetInfo(@ModelAttribute BaseRequestFormExtend formExtend) {
		...
		if (!formExtend.validate()) {
			...
			try {
				...
				BigDecimal totalAssets = new BigDecimal((data.get("totalAssets")).toString());
//				HXBalanceVO cgVO;
//				try {
//					cgVO = hxCommonService.getBalance(formExtend.getUserId());
//				} catch (Exception e) {
//					//华兴账号不存在
//					cgVO = null;
//				}
//
//				data.put("llBalance",data.get("balance"));
//				data.put("balance",new BigDecimal(data.get("balance").toString()).add(null==cgVO?BigDecimal.ZERO:cgVO.getBalance()).toString());
//				data.put("cgBalance",null==cgVO?BigDecimal.ZERO.toString():cgVO.getAvailable().toString());
//				data.put("totalAssets", null==cgVO?totalAssets.toString():totalAssets.add(cgVO.getBalance()).toString());
//				response.setData(data);


				data.put("llBalance",data.get("balance"));
				data.put("balance",new BigDecimal(data.get("balance").toString()).add(BigDecimal.ZERO).toString());
				data.put("cgBalance", BigDecimal.ZERO.toString());
	...
	HttpResponse getUserAccountInfo(@ModelAttribute BaseRequestFormExtend formExtend) {
		...
		if (!formExtend.validate()) {
			...
			if (userAccountInfoVO == null) {
				...
				//HXAccountInfo accountInfo = hxUserService.getAccountInfo(Integer.valueOf(formExtend.getUserId()));
...
//								grantExperienceGold(phoneNum,userId);
								...
								try {
									...
									if (StringUtils.isEmpty(token)) {
										...
										// 返回信息
										Map<String, Object> resultMap = userInfoService.getUserAccountInfoDataMap(userAccountInfoVO,1);
...
//                        userAccountInfoVO.setToken(token.getToken());
						...
						if (StringUtils.isNotBlank(result)) {
							...
							// 获取用户账户信息字段Map
							Map<String, Object> data = userInfoService.getUserAccountInfoDataMap(userAccountInfoVO,1);
							//查询Baofoo绑卡
							...
							if (containBaofoo&&bankList.size()!=0) {
								...
								List<Map<String, Object>> newBankList = userInfoService.getUserBaofooCardInfo(Integer.valueOf((String)(data.get("userId"))));
								Map<String, Object> param = newBankList.get(0);
								String bankNum="";
								try {
									bankNum = StringHelper.decode((String) param.get("bankNum"));
	...
	public HttpResponse messageList(@ModelAttribute BaseRequestForm  paramForm,
		...
		try{
			...
			List<PrivateMessage> list=this.userInfoService.getUserMessage(userId, timestamp, isUp, VersionTypeEnum.PT);
	...
	public HttpResponse messageUnderCount(@ModelAttribute BaseRequestForm  paramForm,
		...
		try{
			...
			//获取用户未读消息数量
			int unreadCount = this.userInfoService.getUserMessageCount(userId, Status.WD.name(), VersionTypeEnum.PT);
	 ...
	 * @return
	 */
	@RequestMapping(value = "platform/information", method = RequestMethod.GET)
