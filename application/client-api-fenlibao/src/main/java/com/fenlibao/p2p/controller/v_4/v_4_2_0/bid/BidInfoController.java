package com.fenlibao.p2p.controller.v_4.v_4_2_0.bid;
 ...
 */
@RestController("v_4_2_0/bidInfoController")
@RequestMapping(value = "bidInfo", headers = APIVersion.v_4_2_0)
public class BidInfoController {
    ...
    public HttpResponse getCaninvestBid(@ModelAttribute BaseRequestForm paramForm,
                ...
                for (ShopTreasureInfo info : canInvestBidList) {
                    ...
                    if (info.getVoteAmount().doubleValue() >= 100 && panicBuyingTime) {
                        ...
                        if (info.getItemType() == 1) {
                            ...
                            if (bidLabels != null && bidLabels.size() > 0) {
                                bidLabels = bidLabels.subList(0, bidLabels.size() >= 2 ? 2 : bidLabels.size());  //标的话显示两个标签
                                vo.setBidLabel(bidLabels);
                            }
                            List<String> markList = new ArrayList<>();
                            if (info.getAccumulatedIncome() != null) {
        ...
        items.addAll(voList);
        if (items.size() < 10){  //少于10条加上历史数据
            List<BidInfoVO> itemsHistory = getHistoryBid(null, null, productType, versionType, 10 - items.size());
            items.addAll(itemsHistory);
        }
    ...
    public List<BidInfoVO> getHistoryBid(String timestamp, String bidOrigin, String productType, String versionType, Integer limit) throws Exception {
        ...
        map.put("cgNum", cgNum);
        map.put("orderByStatus", "1");//先满额后结清
        map.put("fullTime", "1");//满额时间排序
        if (StringUtils.isEmpty(bidOrigin)) {//新
            ...
            if (StringUtils.isNotEmpty(timestamp)) {
                ...
                List<ShopTreasureInfo> plansFullList = this.bidInfoService.getPlansList(status, null, -1, -1, null, null, 1, -1, limit, null, -1, "timeSort", -1, cgNum);
            ...
            if (StringUtils.isNotEmpty(timestamp)) {
                ...
                List<ShopTreasureInfo> plansList = this.bidInfoService.getPlansListOrderBy(status, limit, null, null, cgNum, -1, -1, null);
    ...
    public HttpResponse getBidDetail(@ModelAttribute BaseRequestForm paramForm, String userId, String bidId, String planId, VersionTypeEnum versionType) throws Exception {
        HttpResponse response = new HttpResponse();
        ...
        if (!StringUtils.isBlank(bidId)) {
            ...
            BidBorrowerInfoAfterLoan bidBorrowerInfoAfterLoan = bidInfoService.getBidBorrowerInfoAfterLoan(Integer.valueOf(bidId));
            BidDetailVO vo = bidInfoService.pieceBidDetailVO(info, bbInfo, bidId, userId, id);

            if (versionType.getIndex() == 2) {  //存管才返回信息披露信息
                ...
                vo.setBorrowerInfo(borrowerInfo);
                if ("0001".equals(info.getBidOrigin())){  //来源是分利宝，普通标才显示
                    Map map = bidInfoService.getSignIdAndDocId(bidId);
                    if (map != null && map.get("no_sensitive_sign_id") != null && map.get("no_sensitive_doc_id") != null){
                        vo.setAgreementUrl(ShangshangqianUtil.sdk.ViewContract(map.get("no_sensitive_sign_id").toString(), map.get("no_sensitive_doc_id").toString()));
                    }else {
                        String noSensitiveAgreementPath = bidInfoService.getNoSensitiveAgreementPath(bidId);
                        vo.setAgreementUrl(FileUtils.getFileURL(noSensitiveAgreementPath, Config.get("resources.server.path")));
                    }
                }
            }
            ...
            PlanDetailVO vo = bidInfoService.piecePlanDetailVO(info, planId, userId, id, versionType);
            if (versionType.getIndex() == 2) {  //存管才返回信息披露信息
                List<Map<String, Object>> planInfo = bidInfoService.pieceTogetherPlanInfo(info);
                vo.setPlanInfo(planInfo);
            }
    ...
    @RequestMapping(value = "projectCreditRightList", method = RequestMethod.GET)
    HttpResponse projectCreditRightList(@ModelAttribute BaseRequestForm paramForm, Integer planId, Integer pageIndex, Integer pageSize) throws Exception {
        HttpResponse response = new HttpResponse();
        //参数验证
        if (!paramForm.validate() ||  planId == null ) {
            response.setCodeMessage(ResponseCode.EMPTY_PARAM.getCode(), ResponseCode.EMPTY_PARAM.getMessage());
            return response;
        }
        ...
        Map<String, Object> map = bidInfoService.projectCreditRightList(planId, pageIndex, pageSize);
        response.setData(map);
        return response;
    }
