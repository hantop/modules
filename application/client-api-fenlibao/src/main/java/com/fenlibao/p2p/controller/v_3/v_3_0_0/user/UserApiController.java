...
@RestController("v_3_0_0/UserApiController")
@RequestMapping(value = "user", headers = APIVersion.v_3_0_0)
	...
	HttpResponse getUserAccountInfo(@ModelAttribute BaseRequestFormExtend formExtend) {
		...
		if (!formExtend.validate()) {
			...
			if (userAccountInfoVO == null) {
				...
				data = userInfoService.getUserAccountInfoDataMap(userAccountInfoVO,0);
...
//								grantExperienceGold(phoneNum,userId);
								...
								try {
									...
									if (StringUtils.isEmpty(token)) {
										...
										// 返回信息
										Map<String, Object> resultMap = userInfoService.getUserAccountInfoDataMap(userAccountInfoVO,0);
...
//                        userAccountInfoVO.setToken(token.getToken());
						...
						if (StringUtils.isNotBlank(result)) {
							...
							// 获取用户账户信息字段Map
							Map<String, Object> data = userInfoService.getUserAccountInfoDataMap(userAccountInfoVO,0);
