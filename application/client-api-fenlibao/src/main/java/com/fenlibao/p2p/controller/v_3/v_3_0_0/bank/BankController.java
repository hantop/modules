package com.fenlibao.p2p.controller.v_3.v_3_0_0.bank;
 ...
 */
@RestController("v_3_0_0/BankController")
@RequestMapping(value = "bank", headers = APIVersion.v_3_0_0)
    ...
    public HttpResponse userDealStatus(@ModelAttribute BaseRequestFormExtend paramForm
        ...
        try {
            ...
            if(retMap != null && retMap.size() > 0){
                ...
                boolean flag = (boolean) retMap.get("tradersPwdStatus");
                if(flag){
                    outMap.put("noDealpassword","1");
                }else {
                    outMap.put("noDealpassword","0");
                }

                //获取银行信息卡信息 先根据支行信息判断
                /*BankCardVO bankCardInfo = bankService.checkBankCardInfo(paramForm.getUserId());
                if(bankCardInfo != null){
                    String depositBank = bankCardInfo.getBankKhhName(); //开户行信息是否完整
                    if(StringUtils.isBlank(depositBank)){   //0:银行卡信息没完善  1:银行信息完整
                        retMap.put("bankInfocomplete","0");
                    }else{
                        retMap.put("bankInfocomplete","1");
                    }
                }*/
                BigDecimal llBalance = BigDecimal.ZERO;
                BigDecimal cgBalance = BigDecimal.ZERO;

                if (retMap.get("llBalance") != null) {
                    llBalance = (BigDecimal)(retMap.get("llBalance"));
                }
//                HXBalanceVO cgVO;
//                try {
//                    cgVO = hxCommonService.getBalance(paramForm.getUserId());
//                } catch (Exception e) {
//                    logger.info("****用户存管账户：[userId]-->"+paramForm.getUserId()+" ****message:"+e.getMessage());
//                    cgVO = null;
//                }
//                cgBalance = null==cgVO?BigDecimal.ZERO:cgVO.getAvailable();

                outMap.put("llBalance",llBalance);
                outMap.put("cgBalance", cgBalance);

                outMap.put("balance",llBalance.add(cgBalance));
