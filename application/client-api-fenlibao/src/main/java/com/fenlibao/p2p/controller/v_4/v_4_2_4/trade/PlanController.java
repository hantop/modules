 ...
 * @date 2017年1月22日
 */
@RequestMapping(value = "plan", headers = APIVersion.v_4_2_4)
@RestController("v_4_2_4/PlanController")
               ...
               // BigDecimal expectedProfitPlan = CalCapitalInterestUtil.calTotalInterestYCFQ_DAY(new BigDecimal(infoDetail.getInvestAmount()), infoDetail.getHighRate()), infoDetail.getLoanDate());
                ...
                if(infoDetail.getPlanType().equals("1")){//月月
                    ...
                    if(infoDetail.getInterestTime()!=null) {//开始计息
                        ...
                        } else if (infoDetail.getOwnsStatus() == 3) {//已退出
...
//                    investInfoDetailVO.setPresentRate(infoDetail.getLowRate()+infoDetail.getBonusRate()*month+couponRise);
                    ...
                    investInfoDetailVO.setPresentRate(infoDetail.getLowRate()+infoDetail.getBonusRate()*infoDetail.getCurrentMonth());
                }else {//省心
            ...
            if (Status.HKZ.name().equals(infoDetail.getInvestStatus())) {
                ...
                if(infoDetail.getPlanType().equals("1")){
                    ...
                    if(infoDetail.getOwnsStatus()==0||infoDetail.getOwnsStatus()==1) {
                        ...
                        month = planService.getPlanMonthNum(infoDetail.getOwnsStatus(),interestTime,infoDetail.getExitTime().getTime());
                        rate = new BigDecimal(infoDetail.getLowRate()+infoDetail.getBonusRate()*month+infoDetail.getInterest());
                        expectedProfitPlan   = CalCapitalInterestUtil.calTotalInterestYCFQ_DAY(new BigDecimal(infoDetail.getInvestAmount()), rate, holdDays);
                    }
//                    investInfoDetailVO.setPresentRate(infoDetail.getLowRate()+infoDetail.getBonusRate()*month);
            ...
            //2017-6-6
            investInfoDetailVO.setLowRate(String.valueOf(infoDetail.getLowRate()));
            investInfoDetailVO.setHighRate(String.valueOf(infoDetail.getHighRate()));
            investInfoDetailVO.setBonusRate(String.valueOf(infoDetail.getBonusRate()));
            investInfoDetailVO.setCurrentMonth(infoDetail.getCurrentMonth());
        }
        ...
        if(infoDetail.getIsCG()!=null&&infoDetail.getIsCG().equals("2")){
            investInfoDetailVO.setFwxyUrl(userInfoService.getUserServicePlanAgreementUrlByCG(Integer.valueOf(StringUtils.isBlank(userId) ? "-1" : userId), infoDetail.getIsNoviceBid(), infoDetail.getPlanType()));
        }else {
    ...
    public HttpResponse doPlan( @ModelAttribute BaseRequestFormExtend paramForm, String planId, String sgSum, String isCheck, String fxhbIds, String jxqId, String dealPassword, String newPlan, String versionType)  throws Throwable{
        ...
        try{
            ...
            if("1".equals(newPlan)){
                ...
                if(!VersionTypeEnum.parse(investPlan.getIsCG()).equals(VersionTypeEnum.parse(versionType))){
                    apiResponse.setCodeMessage(ResponseCode.COMMON_PARAM_WRONG);
                    return apiResponse;
                }
                ...
                if(VersionTypeEnum.CG.equals(VersionTypeEnum.parse(investPlan.getIsCG()))){
                    planInfoService.checkCanInvest(planIds,paramForm.getUserId(),VersionTypeEnum.CG);
            ...
            logger.error("出借计划失败，planId=[{}],出借金额=[{}],userId=[{}]", planId, sgSum, paramForm.getUserId());
            logger.error("出借计划失败", throwable);
    ...
    HttpResponse applyforSubmit(@ModelAttribute BaseRequestForm paramForm,
        ...
        HttpResponse response = new HttpResponse();
        try {
            if (!paramForm.validate() || (versionType == VersionTypeEnum.PT&&StringUtils.isEmpty(dealPassword)) || StringUtils.isEmpty(planRecordId)) {
                response.setCodeMessage(ResponseCode.EMPTY_PARAM.getCode(), ResponseCode.EMPTY_PARAM.getMessage());
                return response;
            }
            ...
            if(versionType!=null &&versionType == VersionTypeEnum.PT) {
                //验证用户交易密码
    ...
    public Map<String,Object> validate(PlanRecordInfo planRecordInfo){
        ...
        if(planRecordInfo.getType()!=1) {
            if (null == planRecordInfo.getInterestTime() || InterfaceConst.PLAN_ZQZR_CY_DAY >= DateUtil.getDayBetweenDates(planRecordInfo.getInterestTime(), DateUtil.nowDate())) {
                response.setCodeMessage(ResponseCode.TCJH_SJWD);
                resultMap.put("response", response);
                return resultMap;
            }
        }else {
            if (null == planRecordInfo.getInterestTime() || InterfaceConst.ZQZR_CY_DAY > DateUtil.getDayBetweenDates(planRecordInfo.getInterestTime(), DateUtil.nowDate())) {
                response.setCodeMessage(ResponseCode.TCJH_SJWD);
                resultMap.put("response", response);
                return resultMap;
            }
