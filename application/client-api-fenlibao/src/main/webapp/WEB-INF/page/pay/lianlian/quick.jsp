...
<div class="header">
    ...
    <h1 class="logo">Quick payment Test</h1>
...
<section class="info">
    <table border="0" cellspacing="0" cellpadding="0" class="table_ui">
        <tr>
            ...
            <td style="text-align: right">手机充值</td>
        </tr>
        <tr>
...
<section>
    <div class="form_warp">
        <form id="pay_form" action="https://wap.lianlianpay.com/payment.htm" method="post">
            <ul>
                <li id="post_data"></li>
            </ul>
        </form>
...
<script type="text/javascript">
    $('#next_btn').click(function () {
        ...
        var data = {
            ...
            userId: 8574, isBind: 0,
            version: '1.0.0', deviceId: 1, clientType: 5, screenType: 1, token: 123
        };
        $.ajax({
            url: '<%=request.getContextPath()%>/lianlianPay/quick',
            data: data,
            type: 'post',
            dataType: 'json',
            headers: {'version': '1.4.0'},
            success: function (result) {
                console.info(result);
                if (result.code == '200') {
                    var reqData = result.data.req_data;
                    getDecryptData(reqData);
                } else {
    ...
    function getDecryptData(reqData) {
        $.post('<%=request.getContextPath()%>/lianlianPay/decrypt', {req_data: reqData}, function (result) {
        	console.log(result.req_data);
            $('#post_data').append('<input type="hidden" name="req_data" value=\'' + result.req_data + '\'/>');
            $('#pay_form').submit();
        });
    }
</script>
